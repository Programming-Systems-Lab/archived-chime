...
AIVideoPlayer::AIVideoPlayer(ChimeSystemDriver *csd)
{
	...
	// start timer (will call runTimerLoop)
	...
	// fixed - now can output from cache controller -clCES);
	...
	// fixed - now can output from cache controller -cl
	CacheController = new AICacheController("AI2TV_CACHE", this);
...
void AIVideoPlayer::debug(int level,char* msg){
	...
	//force all messages to stdout
	//if change needed, remove the enforcement
	level = 0;
	try{
	switch(level){
	case 1:
...
// called from AICacheController::processNextFrame
AIVideoFrame*  AIVideoPlayer::createFrame(char* filename, int frame_number, int start, int end){
	...
	//corrected filename -cl
	fr->filename = new char[50];
	sprintf(fr->filename,"%s",filename);
	fr->start=start;
...
DWORD WINAPI runShowFrame(LPVOID lParam){
  ...
  try {
	...
	//	loadFrames(player);
	  ...
	  // wait for download before load first frame -cl
	  while (player->FrameBuffer->size <= 0) {
	    Sleep(60); // wait for first frame to load 
	    // for debugging
	    //	  char msg[50];
	    //	  sprintf(msg,"waiting for frames to load..");
	    //	  player->debug(2,msg);
	  }
	  ...
	  if((myframe==0) && player->FrameBuffer->size >0){
	    ...
	    char msg2[1000];
	    sprintf(msg2,"Frame buffer Size: %d, head=%d.tail=%d",	
		    player->FrameBuffer->size, player->FrameBuffer->getHead(),
		    player->FrameBuffer->tail);
	    player->Sys->Report(CS_REPORTER_SEVERITY_ERROR,msg2);
	    player->debug(1,msg2);
	  }
	  ...
	  if (myframe!=0) {
	    if (myframe->materialWrapper !=0){
	      ...
	      //for debugging -cl
	      char msg[50];
	      sprintf(msg,"Display:%d (%d,%d)", 
		      myframe->frame_number, 
		      myframe->start, myframe->end);
	      player->debug(2,msg);
    ...
    //added error message -cl
    char msg[30];
    sprintf(msg,"error: runShowFrame");
    player->debug(1,msg); 
  }
	return(0);
...
// thread started by runFrameManager 
...
//changed return type from void to inticate session file errors -cl
// called from AIVideoPlayer::getInstance()
//void AIVideoPlayer::Initialize(){
int AIVideoPlayer::Initialize(){
  ...
  if (session=fopen("./data/ai2tv/session.txt","r")){
    // get the # of clients
    if (fscanf(session,"%d",&num_clients) != 1) {
      debug(1,"ERROR: IMPROPER SESSION FILE FORMAT!");
      delete this;
      return -1;
    }
    ...
    for (int x = 0; x < num_clients; ++x){
      ...
      if (fscanf(session,"%d %s",&(clientIDs[x]),ipaddresses[x]) != 2) {
	debug(1,"ERROR: IMPROPER SESSION FILE FORMAT!");
	delete this;
	return -1;
      } 
      // initialize Master Client to the client with the min id number
      //if (clientIDs[x] < clientIDs[minID]) {
      //  minID = x;
      //} 
      // for now just use localhost, and assume client list will be in same order at each client
      if (x == 0 && !strcmp(ipaddresses[x], "localhost")) {
	isMaster = true;
      }
    }

    // now get the Base URL for the JPEGS, and the name of the list file
    if (fscanf(session,"%s",url) != 1 || fscanf(session,"%s",frame_list_file) != 1) {
      debug(1,"ERROR: IMPROPER SESSION FILE FORMAT!");
      delete this;
      return -1;
    }
  ...
  // runFrameManager -- will set up frame arrays 
  runFrameManager();
  return 0;
